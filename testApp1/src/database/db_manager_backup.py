    ...
    def create_scorecard(self, scorecard: Scorecard) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'INSERT INTO scorecards (player_name, date_created, space_read_live_dribble, space_read_catch) VALUES (?, ?, ?, ?)',
                    (scorecard.player_name, scorecard.date_created, scorecard.space_read_live_dribble, scorecard.space_read_catch)
                )
                conn.commit()
                return True
        except Exception as e:
    ...
    def get_scorecards_by_player(self, player_name: str) -> List[Scorecard]:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'SELECT player_name, date_created, space_read_live_dribble, space_read_catch FROM scorecards WHERE player_name = ? ORDER BY date_created DESC',
                    (player_name,)
                )
                rows = cursor.fetchall()
                ...
                scorecards = []
                for row in rows:
                    scorecard = Scorecard(row[0], row[1], row[2], row[3])
